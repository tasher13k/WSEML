...
auto txt = LR"({`rus пы':$})";
static WSEML universe_WSEML = parse(txt); //, `стек':[{`текблк':[{`вид':`двоичный', `адрес':0}], `текком':0, `данные':[]}]}
...
static void resolve()//может быть вызвана как команда, тогда нужен next_cmd, или как процедура, тогда не нужен --- решает регистр кмд
{
    json res, j, j1, *frmo;
    string key;
    int ind;
...
n4:
    ...
    key = regrs(в);
    LOG(RESOLVEL, "resolve_перед заведением нового стекового кадра для пользовательского шага:" STACK<<"   j="<<j<<"   j1="<<j1);
...
n1:
    ...
    name = ref["имя"].get<string>();
    LOG(LOADSTOREL, "store_шаг регистр 2:" STACK<<"  ref="<<ref<<"   name="<<name<<"    frmm(ref)="/*<<frmm(ref)*/<<"    val="<<val);
    frmm(ref)[name] = val;
...
static void label() { next_cmd(); clean(); }
...
user:
    ...
    LOG(EXECL, "exec_cmd_перед запуском пользовательской команды:" STACK<<"   #args="<<universe["таблицы"]["команды"][cmd_name]["арг"]<<endl<<"============================="<<endl<<universe);
    for(int i = 0; i<universe["таблицы"]["команды"][cmd_name]["арг"].get<int>(); ++i)
    {
        push((*frmo)["данные"][(*frmo)["данные"].size()-universe["таблицы"]["команды"][cmd_name]["арг"].get<int>()+i]);
        (*frmo)["данные"].erase((*frmo)["данные"].size()-universe["таблицы"]["команды"][cmd_name]["арг"].get<int>()+i);
    }
...
void debug()
{
    ...
    while(universe["стек"].size()>0)
    {
        ...
        {
            case 0: return;
            case 1: cout<<"============================ очередной шаг =================================="<<endl;
                    try
                    {
                        if(!lfrm()["текблк"].is_string())
                        {
